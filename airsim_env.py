...
class AirSimEnv_v3(gym.Env):
    def __init__(self, reward_path='./model/airl/reward/reward_net65.pth'):
        ...
        # self.reward_net_old = torch.load('./model/airl/reward/reward_net65.pth',
        #                                  weights_only=False,
        #                                  map_location=torch.device("cuda:0"))
        # self.reward_net_old.eval()

        # self.reward_net = torch.load(reward_path, weights_only=False, map_location=torch.device("cuda:0"))
        # self.reward_net.eval()

        self.distance = []
        self.max_distance = 0
        self.episode_distance = 0
        # print(f'successfully load {reward_path}')
    ...
    def reset(self, seed=None, options=None):
        ...
        # self.client.moveByVelocityAsync(x, 0, y, 2 * timeslice).join()
    ...
    def get_state(self):
        while True:
            ...
            request = [airsim.ImageRequest(1, airsim.ImageType.DepthVis, True)]
            responses = self.client.simGetImages(request)
            img1d = np.array(responses[0].image_data_float, dtype=np.float32)
            h = responses[0].height
            w = responses[0].width
            # print(h, w, img1d.size)
            if img1d.size != h * w:
                print(f"Invalid image size: {img1d.size}! expected {h * w}. Retry after 5s.")
                time.sleep(5)
                continue
    ...
    def step(self, action):
        ...
        # log info
        info = {}
        distance = self.client.getMultirotorState().kinematics_estimated.position.y_val
        ...
        if done:
            ...
            if self.episode % 100 == 0:
                print(
                    f'episode distance: {self.episode_distance} maxdistance: {self.max_distance} meandistance: {np.mean(self.distance)}')
            self.episode_distance = 0
